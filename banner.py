...
warnings.filterwarnings(
    "ignore", message="textsize is deprecated and will be removed in Pillow 10"
)
...
def make_booth_text(resize=1):
    ...
    image.paste(
        footer,
        ((image.size[0] - footer.size[0]) // 2, maya.size[1] + header.size[1] - 8),
        footer,
    )
    image.paste(
        header, ((image.size[0] - header.size[0]) // 2, maya.size[1] - 10), header
    )
    image_box = image.getbbox()
    image = image.crop(image_box)
    image = image.resize(
        (int(image.size[0] * resize), int(image.size[1] * resize)), Image.Resampling.BOX
    )
    return image
