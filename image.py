# class takes 4 images then
## resize and crop
## creates photobooth pdf/jpg
## prints
## resize and crop for ig
## upload to ig
import os
...
class Booth:
    ...
    def __init__(self, image_dir, image_size=(750, 500), square_size=1080, top_margin=50):
    ...
    def assemble(self):
        ...
        half.paste(
            BOOTH_TEXT,
            ((half.size[0] - BOOTH_TEXT.size[0]) // 2, (half.size[1] - BOOTH_TEXT.size[1]) // 2),
            BOOTH_TEXT,
        )
        half.save(os.path.join(self.image_dir, 'booth.png'))
        full = Image.new("RGBA", (half.size[0], half.size[1]*2 + self.top_margin*2))
        full.paste(half, (0, 0))
        full.paste(half, (0, half.size[1] + self.top_margin*2))
